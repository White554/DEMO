      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt"  v-model="goodNum"  @change="goodNumInput"/>
                <a href="javascript:" class="plus" @click="goodNum++">+</a>
                <a href="javascript:" class="mins" @click="reduceGoodNum">-</a>
              </div>
              <div class="add">
  ...
  methods:{
    ...
    goodNumInput(e) {
      ...
      if (isNaN(num) || num < 1) {
        ...
        // 对数字取证，排除小数
        this.goodNum = Number.parseInt(num);
      }
    },
    // 添加至购物车
    async addCar() {
      // 1 发请求至服务器
      // let result = await this.$store.dispatch('detailStore/getPostGoodCar', {skuId:this.skuInfo.id, skuNum:this.goodNum})
      // if (result.code === 200) {
      //   console.log(result);
      // }
      // 2 服务器存储成功进行路由跳转
      // 3 路由跳转带参数
      try {
        await this.$store.dispatch('detailStore/getPostGoodCar', {skuId:this.skuInfo.id, skuNum:this.goodNum})
